...
def test_clipped_around_zero_bijector() -> None:
    rng = jax.random.PRNGKey(1)
    mean = jax.random.uniform(rng, (DISTRIBUTION_SIZE,), minval=-2.0, maxval=2.0)
    std = jax.random.uniform(rng, (DISTRIBUTION_SIZE,), minval=0.01, maxval=0.1)
    ...
    # Test inverse transformation
    x_recon, log_det_inv = bijector.inverse_and_log_det(y)
    chex.assert_trees_all_close(y, x_recon, atol=1e-6)  # For clipping, inverse should return clipped value
