...
__all__ = [
    ...
    "ReferenceMotionReward",
    "TimeWarpedReferenceMotionReward",
    "JoystickReward",
]
...
@attrs.define(frozen=True, kw_only=True)
class ReferenceMotionReward(Reward):
    reference_motion: xax.HashableArray
    ctrl_dt: float
    observation_name: str
    norm: xax.NormType = attrs.field(default="l1")
    sensitivity: float = attrs.field(default=5.0)
    joint_weights: xax.HashableArray | None = attrs.field(default=None)
    ...
    def get_reward(self, trajectory: Trajectory) -> Array:
        if self.observation_name is None:
            raise ValueError("Observation name is not set; set it as an observation in your task.")
        ...
        error = xax.get_norm(reference_motion[..., 7:] - observed_motion, self.norm)
        if self.joint_weights:
            error = error * self.joint_weights
...
@attrs.define(frozen=True, kw_only=True)
class TimeWarpedReferenceMotionReward(Reward):
    reference_motion: xax.HashableArray
    ctrl_dt: float
    sensitivity: float = attrs.field(default=5.0)
