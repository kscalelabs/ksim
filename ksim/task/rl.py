...
class RLTask(xax.Task[Config, InitParams], Generic[Config], ABC):
    ...
    def _log_logged_trajectory_graphs(
        ...
        arrs: list[tuple[str, Mapping[str, Array]]] = [
            ("🎁 reward images", {f"x/{k}": v for k, v in logged_traj.rewards.components.items()}),
    ...
    def get_reward_metrics(self, trajectories: Trajectory, rewards: RewardState) -> dict[str, Array]:
        ...
        return {
            "total": rewards.total,
            **{f"x/{key}": value for key, value in rewards.components.items()},
        }
            ...
            try:
                for _ in iterator:
                    ...
                    reward_scalars = {
                        ...
                        **{f"x/{k}": float(jax.device_get(v[-1])) for k, v in reward_state.components.items()},
                    }
