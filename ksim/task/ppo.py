...
from ksim.task.rl import MinibatchedRLTask, RLConfig, RLLoopCarry, RLLoopConstants
...
class PPOTask(MinibatchedRLTask[Config], Generic[Config], ABC):
    ...
    def _get_ppo_loss_and_metrics(
        ...
        def loss_and_metrics_fn(
            ...
            off_policy_variables, _ = self.get_training_variables(model, trajectory, init_model_carry, rng2)
            assert isinstance(off_policy_variables, PPOVariables)
